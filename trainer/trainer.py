...
class Trainer(BaseTrainer):
    ...
    def __init__(self, model, loss_ftns, optimizer, config, data_loader,
        ...
        if len_epoch is None:
            ...
            self.len_epoch = len(self.data_loader)  #返回1个 epoch 的长度
        else:
            # iteration-based training
            self.data_loader = inf_loop(data_loader)    #无限循环加载数据
        ...
        mt_keys = ['loss']
        for data_source in data_sources:    #data_sources = ('esim', 'ijrr', 'mvsec', 'eccd', 'hqfd', 'unknown')
            mt_keys.append(f'loss/{data_source}')   #f接{}可动态表示，如loss/esim
            for l in self.loss_ftns:                #perceptual_loss/esim
                mt_keys.append(f'{l.__class__.__name__}/{data_source}')
        self.train_metrics = MetricTracker(*mt_keys, writer=self.writer)    #传递指标名称，可以方便地在训练过程中更新和记录这些指标的数值
        self.valid_metrics = MetricTracker(*mt_keys, writer=self.writer)

        self.num_previews = config['trainer']['num_previews']   #dictionary.get(key, default)
        self.val_num_previews = config['trainer'].get('val_num_previews', self.num_previews)    #验证集分为多少部分
        self.val_preview_indices = select_evenly_spaced_elements(self.val_num_previews, len(self.valid_data_loader))    #选择出均匀间隔的元素的索引值，并以列表的形式返回
    ...
    def forward_sequence(self, sequence, all_losses=False):
        losses = collections.defaultdict(list)#创建一个默认值为列表的字典
        self.model.reset_states()   #状态置零
        for i, item in enumerate(sequence):
            ...
            for loss_ftn in self.loss_ftns:
                ...
                loss_ftn.weight = tmp_weight    #the index of the data source (simulated, IJRR, MVSEC etc）
        idx = int(item['data_source_idx'].mode().values.item()) #返回众数的值
        data_source = data_sources[idx]
        losses = {f'{k}/{data_source}': mean(v) for k, v in losses.items()}#loss名/数据集：损失值
