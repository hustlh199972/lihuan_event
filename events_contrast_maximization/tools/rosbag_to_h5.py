...
#C:\Users\10123\Desktop\E2VID\data\rosbag\frames.bag
'''
python events_contrast_maximization/tools/rosbag_to_h5.py rosbag/still_life.bag --output_dir data/h5  --event_topic /dvs/events --image_topic /dvs/image_raw
python events_contrast_maximization/tools/rosbag_to_h5.py data/rosbag/sun7.bag --output_dir data/h5  --event_topic /dvs/cam1/events  --image_topic /dvs/image_reconstructed
vitables
'''
...
def get_rosbag_stats(bag, event_topic, image_topic=None, flow_topic=None):
    ...
    num_img_msgs = 0
    num_flow_msgs = 0
    topics = bag.get_type_and_topic_info().topics   #返回一个字典，其中键是话题名称，对应的值是一个元组
    print('topics内容:{}'.format(topics))
    for topic_name, topic_info in topics.items():
        print('topic_name:{} topic_info:{}'.format(topic_name,topic_info))
...
def extract_rosbag(rosbag_path, output_path, event_topic, image_topic=None,
    ...
    ep = packager(output_path)  #在output_path创建h5文件，写入空的xs、ys、ts、ps
...
def extract_rosbags(rosbag_paths, output_dir, event_topic, image_topic, flow_topic,
    ...
    for path in rosbag_paths:
        bagname = os.path.splitext(os.path.basename(path))[0]#获取路径中的文件名部分
