...
class BaseVoxelDataset(Dataset):
    ...
    def __getitem__(self, index, seed=None):
        ...
        idx0, idx1 = self.get_event_indices(index)  # frame是一个H x W的张量，其中包含时间戳大于等于事件张量的第一帧
                                                    # events是一个C x H x W的张量，其中包含体素网格
                                                    # flow是一个2 x H x W的张量，其中包含从当前帧到上一帧的光流（位移）
        ...
        if self.voxel_method['method'] == 'between_frames':
            frame = self.get_frame(index)   #这是父类的方法，后面会用子类的方法
...
class DynamicH5Dataset(BaseVoxelDataset):##实例化时也传入了参数

    def get_frame(self, index):
        return self.h5_file['images']['image{:09d}'.format(index)][:]

    def get_flow(self, index):
        return self.h5_file['flow']['flow{:09d}'.format(index)][:]  #[:]不影响原始列表
    ...
    def load_data(self, data_path):
        try:            #h5py库提供的用于打开HDF5格式文件的函数
            # print('load_data函数检查：data_path为：{}'.format(data_path))
            self.h5_file = h5py.File(data_path, 'r')
        except OSError as err:
            print("Couldn't open {}: {}".format(data_path, err))
        if self.sensor_resolution is None:  #使用attrs属性来访问文件的属性信
        ...
        self.t0 = self.h5_file['events/ts'][0]  #t0 - 第一个事件的时间戳
        self.tk = self.h5_file['events/ts'][-1] #tk - 最后一个事件的时间戳
        self.num_events = self.h5_file.attrs["num_events"]
        self.num_frames = self.h5_file.attrs["num_imgs"]

        self.frame_ts = []  #把事件时间帧加入到列表
...
class SequenceDataset(Dataset): #实例化时还传入了参数
    """Load sequences of time-synchronized {event tensors + frames} from a folder."""
    def __init__(self, data_root, sequence_length, dataset_type='DynamicH5Dataset',
        ...
        self.dataset = eval(dataset_type)(data_root, **dataset_kwargs)  #eval执行一个字符串表达式，并返回表达式的值
    ...
    def __getitem__(self, i):   #变为迭代器后会自动调用，不用输入i
        ...
        k = 0
        j = i * self.step_size
        item = self.dataset.__getitem__(j, seed)    #BaseVoxelNet的__getitem__方法，返回item
